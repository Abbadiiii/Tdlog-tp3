...
class BattlefieldEntity(Base):
    ...
    players = relationship("PlayerEntity", back_populates="battle_field")
    vessel=ralationship("VesselEntity",back_populates="battlefield",uselist=False, cascade="all, delete-orphan")
...
class WeaponEntity(Base):
    ...
    battle_field = relationship("WeaponEntity",back_populates="weapon",uselist=False, cascade="all, delete-orphan")
...
class PlayerDao :
    ...
    def create_player(self, player:Player, Game_id:int) -> int:
        player_entity = map_to_game_entity(player,game_id)
        self.db_session.add(player_entity)
        self.db_session.commit()
        return game_entity.id
    def find_game(self, player_id: int) -> Player:
        stmt = select(PlayerEntity).where(PlayerEntity.id == player_id)
        player_entity = self.db_session.scalars(stmt).one()
        return map_to_game(player_entity)    
        
class VisselDao :
    def __init__(self):
        Base.metadata.create_all()
        self.db_session = Session()
    def create_vessel(self, vessel:Vessel,player_id: int) -> int:
        vessel_entity = map_to_vessel_entity(vessel,player_id)
        self.db_session.add(vessel_entity)
        self.db_session.commit()
        return vessel_entity.id
    def find_game(self, vessel_id: int) -> Vessel:
        stmt = select(VesselEntity).where(VesselEntity.id == vessel_id)
        gvessel_entity = self.db_session.scalars(stmt).one()
        return map_to_game(vessel_entity)
